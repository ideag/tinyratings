...
// [ ] TO DO: readme.txt.
// [ ] TO DO: auto append to post content / term description(?).
// [ ] TO DO: shortodes for top lists.
// [ ] TO DO: widgets.
// initialize plugin.
...
class TinyRatings {
	...
	public static function api_create( $request ) {
		...
		if ( 0 < count( $result['current'] ) ) {
			if ( ! self::$options['allow_change'] ) {
				$result['error'] = new WP_Error( 'tinyratings_duplicate', esc_html__( 'Your rating is already in the database!', 'tinyratings' ), array( 'status' => 406 ) );
			}
	...
	public static function shortcode( $atts = array(), $content = '' ) {
		...
		$return = '<div class="' . $container_class . '" data-style="' . $atts['style'] . '" data-object-id="' . $atts['id'] . '" data-object-type="' . $atts['type'] . '">' . $return . '</div>';
		return $return;
	}
	/**
